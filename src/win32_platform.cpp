...
LRESULT CALLBACK windows_window_callback(HWND window, UINT msg,
  ...
  switch (msg)
  {
    ...
    default:
    {
       //Let windiws handle the default input for now
...
bool platform_create_window(int width, int height, const char* title){
    ...
    // WS_CAPTION | WS_SYSMENU | WS_THICKFRAME | WS_MINIMIZEBOX | WS_MAXIMIZ
...
void* platform_load_gl_function(const char* funName) {
    ...
    if (!proc) {
        static HMODULE openglDLL = LoadLibraryA("opengl32.dll");  // Correção aqui
        ...
        if (!proc) {
            SM_ASSERT(false, "Failed to load gl function %s", funName); // Correção aqui
            return nullptr;
        }
    }
