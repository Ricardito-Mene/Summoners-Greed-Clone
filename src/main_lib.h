...
inline void _log(const char* prefix, const char* msg, TextColor textColor)
{
    ...
    snprintf(textBuffer, sizeof(textBuffer), "%s %s %s \033[0m", TextColorTable[textColor], prefix, msg);

    puts(textBuffer);
}

template <typename ...Args>
inline void _log(const char* prefix, const char* msg, TextColor textColor, Args... args)
{
    static const char* TextColorTable[TEXT_COLOR_COUNT] = 
    {    
        "\x1b[30m", // TEXT_COLOR_BLACK
...
inline void free_bump_allocator(BumpAllocator* allocator)
{
